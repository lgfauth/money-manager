...
internal sealed class FinancialHealthSnapshotWorker(
    ILogger<FinancialHealthSnapshotWorker> logger,
    ...
    IServiceScopeFactory scopeFactory) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        logger.LogInformation(
            "FinancialHealthSnapshotWorker INICIADO — Agendado para {Hour:00}:{Minute:00} ({TimeZone}) | Loop: {LoopDelay}s",
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                var runtimeSchedule = await commandQueue.GetScheduleStateAsync(nameof(FinancialHealthSnapshotWorker));
                ...
                var claimedCommand = await commandQueue.ClaimNextCommandAsync(nameof(FinancialHealthSnapshotWorker), nameof(FinancialHealthSnapshotWorker));
                if (claimedCommand != null)
                {
                    if (string.Equals(claimedCommand.CommandType, "pause", StringComparison.OrdinalIgnoreCase))
                    {
                        await commandQueue.SetPausedStateAsync(nameof(FinancialHealthSnapshotWorker), true, nameof(FinancialHealthSnapshotWorker));
                    ...
                    {
                        await commandQueue.SetPausedStateAsync(nameof(FinancialHealthSnapshotWorker), false, nameof(FinancialHealthSnapshotWorker));
                        await commandQueue.CompleteAsync(claimedCommand.CommandId, true, null);
                    }
                    else if (string.Equals(claimedCommand.CommandType, "run-now", StringComparison.OrdinalIgnoreCase))
                    {
                        var pauseState = await commandQueue.GetPauseStateAsync(nameof(FinancialHealthSnapshotWorker));
                ...
                var isPaused = (await commandQueue.GetPauseStateAsync(nameof(FinancialHealthSnapshotWorker))).IsPaused;
                if (isPaused)
                {
            ...
            {
                logger.LogError(ex, "Erro inesperado no loop do FinancialHealthSnapshotWorker.");
            }

            var dynamicDelay = (await commandQueue.GetScheduleStateAsync(nameof(FinancialHealthSnapshotWorker)))?.LoopDelaySeconds ?? _schedule.LoopDelaySeconds;
            await timeProvider.Delay(TimeSpan.FromSeconds(dynamicDelay), stoppingToken);
        }

        logger.LogInformation("FinancialHealthSnapshotWorker finalizado.");
    }
    ...
    private async Task<(bool Success, string? ErrorMessage)> RunOnceAsync(DateTimeOffset runStartedAtUtc, CancellationToken stoppingToken, string triggerType)
    {
        ...
        var processor = scope.ServiceProvider.GetRequiredService<FinancialHealthSnapshotProcessor>();

        processLogger.Start("FinancialHealthSnapshot", new Dictionary<string, object?>
        {
            ["source"] = "Worker",
            ["worker"] = nameof(FinancialHealthSnapshotWorker),
